...
class TrainMolecularMetricsDiscrete(nn.Module):
    def __init__(self, dataset_infos):
        ...
        # Separate metrics for per-batch logging
        self.batch_atom_metrics = AtomMetricsCE(dataset_infos=dataset_infos)
        self.batch_bond_metrics = BondMetricsCE()
    ...
    def forward(self, masked_pred_X, masked_pred_E, true_X, true_E, log: bool):
        ...
        self.batch_atom_metrics(masked_pred_X, true_X)
        self.batch_bond_metrics(masked_pred_E, true_E)
        
        if log:
            to_log = {}
            for key, val in self.batch_atom_metrics.compute().items():
                to_log['train/' + key] = val.item()
            for key, val in self.batch_bond_metrics.compute().items():
                to_log['train/' + key] = val.item()
            if wandb.run:
                wandb.log(to_log, commit=False)

            # Only reset batch-level metrics for per-batch logging
            self.batch_atom_metrics.reset()
            self.batch_bond_metrics.reset()

    def reset(self):
        for metric in [self.train_atom_metrics, self.train_bond_metrics, self.batch_atom_metrics, self.batch_bond_metrics]:
            metric.reset()
