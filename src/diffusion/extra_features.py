...
def _sanitize_symmetric_matrix(mat: torch.Tensor) -> torch.Tensor:
    """Numerically sanitize a batched symmetric matrix before eigendecomposition."""
    mat = 0.5 * (mat + mat.transpose(-1, -2))
    mat = torch.nan_to_num(mat, nan=0.0, posinf=1e6, neginf=-1e6)
    return mat


def _safe_batch_eigh(mat: torch.Tensor):
    """
    Robust batched eigh with jitter retries and per-sample fallback.
    Returns eigenvalues/eigenvectors with same device and dtype as input.
    """
    mat = _sanitize_symmetric_matrix(mat)
    eye = torch.eye(mat.size(-1), device=mat.device, dtype=mat.dtype).unsqueeze(0)
    jitters = (0.0, 1e-7, 1e-6, 1e-5, 1e-4)

    # Fast path: full batched decomposition.
    for eps in jitters:
        try:
            cur = mat if eps == 0.0 else mat + eps * eye
            return torch.linalg.eigh(cur)
        except RuntimeError:
            continue

    # Fallback: decompose each sample independently.
    evals = []
    evecs = []
    for idx in range(mat.size(0)):
        m = mat[idx]
        ok = False
        # Try current device first, then float64 on CPU.
        for to_cpu64 in (False, True):
            cur_base = m
            if to_cpu64:
                cur_base = m.detach().to(dtype=torch.float64, device="cpu")
                eye_local = torch.eye(cur_base.size(-1), dtype=cur_base.dtype, device=cur_base.device)
            else:
                eye_local = torch.eye(cur_base.size(-1), dtype=cur_base.dtype, device=cur_base.device)

            for eps in jitters:
                try:
                    cur = cur_base if eps == 0.0 else cur_base + eps * eye_local
                    ev, evec = torch.linalg.eigh(cur)
                    if to_cpu64:
                        ev = ev.to(dtype=mat.dtype, device=mat.device)
                        evec = evec.to(dtype=mat.dtype, device=mat.device)
                    evals.append(ev)
                    evecs.append(evec)
                    ok = True
                    break
        ...
        if not ok:
            # Last-resort fallback to keep training alive.
            n = m.size(0)
            evals.append(torch.zeros(n, device=mat.device, dtype=mat.dtype))
            evecs.append(torch.eye(n, device=mat.device, dtype=mat.dtype))

    return torch.stack(evals, dim=0), torch.stack(evecs, dim=0)


def _safe_batch_eigvalsh(mat: torch.Tensor) -> torch.Tensor:
    """Robust batched eigvalsh using _safe_batch_eigh fallback."""
    mat = _sanitize_symmetric_matrix(mat)
    jitters = (0.0, 1e-7, 1e-6, 1e-5, 1e-4)
    eye = torch.eye(mat.size(-1), device=mat.device, dtype=mat.dtype).unsqueeze(0)
    for eps in jitters:
        try:
            cur = mat if eps == 0.0 else mat + eps * eye
            return torch.linalg.eigvalsh(cur)
        except RuntimeError:
            continue
    evals, _ = _safe_batch_eigh(mat)
    return evals
...
class EigenFeatures:
    ...
    def __call__(self, noisy_data):
        ...
        if self.mode == 'eigenvalues':
            eigvals = _safe_batch_eigvalsh(L)        # bs, n
            ...
            eigvals, eigvectors = _safe_batch_eigh(L)
            eigvals = eigvals.type_as(A) / torch.sum(mask, dim=1, keepdim=True)
...
def get_eigenvalues_features(eigenvalues, k=5):
    ...
    # Handle cases where there are no connected components (e.g., empty graphs)
