...
class DiscreteUniformTransition:
    ...
    def get_Qt_from_alpha(self, alpha_t, device):
        """Return transition matrices parameterized by the retained-signal mass alpha_t."""
        ...
        q_x = alpha_t * torch.eye(self.X_classes, device=device).unsqueeze(0) + (1 - alpha_t) * self.u_x
        q_e = alpha_t * torch.eye(self.E_classes, device=device).unsqueeze(0) + (1 - alpha_t) * self.u_e
        q_y = alpha_t * torch.eye(self.y_classes, device=device).unsqueeze(0) + (1 - alpha_t) * self.u_y
    ...
    def get_Qt(self, beta_t, device):
        ...
        """
        return self.get_Qt_from_alpha(1 - beta_t, device)
    ...
    def get_Qt_bar(self, alpha_bar_t, device):
        ...
        returns: qx (bs, dx, dx), qe (bs, de, de), qy (bs, dy, dy).
        """
        return self.get_Qt_from_alpha(alpha_bar_t, device)
