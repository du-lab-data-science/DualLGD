...
def process_spec_file(meta, tuples, precision=4, max_inten=0.001, max_peaks=60):
    ...
    # Check if merged_spec is empty after filtering
...
def assign_subforms(form, spec, ion_type, mass_diff_thresh=15):
    ...
    """
    from .chem_utils import standardize_adduct  # Import standardize_adduct
    
    # Standardize the ion type to ensure it's in the supported list
    ion_type = standardize_adduct(ion_type)
...
def get_output_dict(
    ...
    try:
        # Try to standardize the ion type
        standardized_ion_type = standardize_adduct(ion_type)
        if spec is not None and standardized_ion_type in ION_LST:
            output_dict = assign_subforms(
                form, spec, ion_type, mass_diff_thresh=mass_diff_thresh
            )
    except ValueError as e:
        # If ion type is not supported, leave output_tbl as None
        print(f"Warning: Unsupported ion type '{ion_type}': {e}")
