...
ion_remap.update(
    {
        ...
        "M-2H2O+H": "[M-H4O2+H]+",
        "[M-2H2O+H]+": "[M-H4O2+H]+",
        "[M+H-H2O]+": "[M-H2O+H]+",  # Add NIST mapping
        "[M+H-2H2O]+": "[M-H4O2+H]+",  # Add NIST mapping
        "[M+2Na-H]+": "[M-H+2Na]+",  # Add SRM mapping
    }
)
...
ion_to_mass = {
    ...
    "[M-H+2Na]+": -ELEMENT_TO_MASS["H"] + 2 * ELEMENT_TO_MASS["Na"] - ELECTRON_MASS,  # Add NIST ion mass
}
...
ion_to_add_vec = {
    ...
    "[M-H+2Na]+": -element_to_position["H"] + 2 * element_to_position["Na"],  # Add NIST ion vector
}
