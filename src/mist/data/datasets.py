...
def get_paired_spectra(
    ...
    **kwargs,
    ...
    # Use the order from labels.tsv instead of filesystem glob order
    if spec_folder is not None and spec_folder.exists():
        # Create a mapping of spec names to file paths
        all_files = [Path(i).resolve() for i in spec_folder.glob("*.ms")]
        name_to_file = {f.stem: f for f in all_files}
        
        # Build spectra_files list following the order in labels.tsv
        spectra_files = []
        for spec_name in compound_id_file["spec"]:
            if spec_name in name_to_file:
                spectra_files.append(name_to_file[spec_name])
    else:
        logging.info(
            f"Unable to find spec folder {str(spec_folder)}, adding placeholders"
        )
        spectra_files = [Path(f"{i}.ms") for i in compound_id_file["spec"]]
    ...
    # Get file name from Path obj - use stem to properly handle filenames with dots
    get_name = lambda x: x.stem
