...
class Featurizer(ABC):
    ...
    def _resolve_path(file_path: str) -> Path:
        """Resolve a file path, handling Hydra's working directory change."""
        p = Path(file_path)
        if p.exists():
            return p
...
class GraphFeaturizer(Featurizer):
    ...
    def _featurize(self, obj: object) -> Data:
        ...
        graph_data = Data(x=x, edge_index=edge_index, edge_attr=edge_attr, y=y, inchi=inchi)
