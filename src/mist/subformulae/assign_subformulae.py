...
def assign_subforms(spec_files, labels_file,
    ...
    valid_input_specs = [(name, spec) for name, spec in input_specs if spec is not None]
    input_specs_dict = {tup[0]: tup[1] for tup in valid_input_specs}
    export_dicts, spec_names = [], []
    for _, row in labels_df.iterrows():
        spec = str(row["spec"])
        
        # Skip specs that were filtered out due to empty spectra
