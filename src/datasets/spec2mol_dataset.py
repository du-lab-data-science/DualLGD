...
def get_encoder_fp_names(cfg):
    fp_names = getattr(cfg.dataset, 'encoder_fp_names', ['morgan4096'])
    if isinstance(fp_names, str):
        return [fp_names]
    return list(fp_names)


def get_encoder_fp_size(cfg):
    return featurizers.FingerprintFeaturizer.get_fingerprint_size(
        get_encoder_fp_names(cfg)
    )
...
class Spec2MolDataModule(MolecularDataModule):
    def __init__(self, cfg):
        ...
        self.train_smiles = []
        encoder_fp_names = get_encoder_fp_names(cfg)
        ...
        paired_featurizer = featurizers.PairedFeaturizer(
            ...
            mol_featurizer=featurizers.FingerprintFeaturizer(fp_names=encoder_fp_names, **cfg.dataset),
        ...
        # Save the current order of the test set
        pathlib.Path(cfg.general.parent_dir + '/splits/').mkdir(parents=True, exist_ok=True)
        with open(cfg.general.parent_dir + f'/splits/{cfg.general.name}_test_split.txt', 'w') as f:
            for spec, mol in test:
                spec_str = spec.get_spec_name()
                mol_str = mol.get_smiles()
                f.write(f"{spec_str}\t{mol_str}\n")
