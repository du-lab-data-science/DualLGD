...
from src.diffusion.inference_sampling import build_inference_sampling_plan, get_reverse_sampling_transitions
...
class FP2MolDenoisingDiffusion(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, visualization_tools, extra_features,
        ...
        model_name = str(self.cfg.model.get('model', 'graph_tf')).lower()
        backbone = str(self.cfg.model.get('backbone', '')).lower()

        y_fp_dim = cfg.dataset.morgan_nbits
        ...
        self._model_compile_attempted = False
    ...
    def on_validation_epoch_end(self) -> None:
        ...
        if self.global_rank == 0:
            ...
            if self._val_profile is not None and self._val_epoch_wall_start is not None:
                ...
                logging.info(
                    "[Profile][ValCPU][rank=%d] wall=%.2fs mol_from_graphs=%.2fs (%d calls, %.1f%% hotspot, %.1f%% wall) pickle.dump=%.2fs (%d calls, %.1f%% hotspot, %.1f%% wall)",
                    self.global_rank,
                    val_wall_seconds,
                    mol_seconds,
    ...
    def test_step(self, data, i):
        ...
        self._profiled_pickle_dump(f"preds/{self.name}_rank_{self.global_rank}_pred_{i}.pkl", predicted_mols)
        self._profiled_pickle_dump(f"preds/{self.name}_rank_{self.global_rank}_true_{i}.pkl", true_mols)
    ...
    def on_test_epoch_end(self) -> None:
        ...
        if self.global_rank == 0:
            ...
            if self._test_profile is not None and self._test_epoch_wall_start is not None:
                ...
                logging.info(
                    "[Profile][TestCPU][rank=%d] wall=%.2fs mol_from_graphs=%.2fs (%d calls, %.1f%% hotspot, %.1f%% wall) pickle.dump=%.2fs (%d calls, %.1f%% hotspot, %.1f%% wall)",
                    self.global_rank,
                    test_wall_seconds,
                    mol_seconds,
    ...
    def _should_use_compiled_model(self) -> bool:
    ...
    def _get_model_for_current_mode(self):
        if not self._should_use_compiled_model():
            return self.model
        ...
        if self._model_compile_attempted:
            return self.model

        self._model_compile_attempted = True
        try:
            self._compiled_model = torch.compile(self.model, mode="default")
            logging.info("Compiled model with torch.compile for test/predict inference.")
            if hasattr(torch, "compiler") and hasattr(torch.compiler, "cudagraph_mark_step_begin"):
                torch.compiler.cudagraph_mark_step_begin()
            return self._compiled_model
        except Exception as e:
            logging.warning(f"Failed to compile model for inference, falling back to eager mode: {e}")
            return self.model
    ...
    def forward(self, noisy_data, extra_data, node_mask):
        ...
        model = self._get_model_for_current_mode()
    ...
    def sample_batch(self, batch: Batch) -> list[Chem.Mol]:
        ...
        for s_int, t_int in tqdm(sampling_pairs, desc='Sampling', leave=False):
            s_array = s_int * torch.ones((len(batch), 1), dtype=torch.float32, device=self.device)
            t_array = t_int * torch.ones((len(batch), 1), dtype=torch.float32, device=self.device)
        ...
        for nodes, adj_mat in zip(X, E):
            mols.append(self._profile_test_mol_from_graphs(nodes, adj_mat))
